...
const RelatedProductsCarousel = ({list}) => {
          ...
          {list.map((item, index) => (
            <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/4 pl-4">
              <Link to={`/product/${item._id}`} key={index}>
                <div className="bg-center bg-cover p-4 h-[450px] flex flex-col justify-between rounded-xl relative hover:shadow-lg hover:scale-[1.02] duration-200">
                  <img src={item.images[0]} crossOrigin="anonymous" className="absolute w-full h-full top-0 left-0 rounded-xl object-cover" alt="" />
                  ...
                  <div className="bg-white p-2 rounded-xl relative">
                    <h4 className="text-darkGrey font-semibold text-lg mb-1 truncate">{item.title}</h4>
